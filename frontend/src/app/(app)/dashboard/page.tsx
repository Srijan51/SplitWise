...
export default function DashboardPage() {
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Fetch full details and balances for each group in parallel
        ...
        for (const item of results) {
          if (!item) continue;
          const { groupData, balData } = item;
          fullGroups.push(groupData);
          newBalances[groupData.id] = balData;

          if (groupData.expenses) {
            groupData.expenses.forEach((exp: any) => {
              const mySplit =
                exp.splits?.find((s: any) => s.userId === userData.id)?.amountOwed || 0;
  ...
  useEffect(() => {
    ...
    socket.on("expense_added", handleUpdate);
    socket.on("expense:created", handleUpdate);
    socket.on("balance:updated", handleUpdate);

    return () => {
      socket.off("expense_added", handleUpdate);
      socket.off("expense:created", handleUpdate);
  ...
  if (session) {
    Object.values(balances).forEach((bal: any) => {
      const myBal =
        bal.memberBalances?.find((m: any) => m.userId === session.user.id)?.netBalance || 0;
      totalOwed += myBal;
    });
  }
    ...
    <div className="mt-10 md:mt-8">
      <div className="flex items-center justify-between px-6 md:px-8 mb-4">
        ...
        <button
          ...
          className="text-[13px] font-bold text-[#335c52] flex items-center gap-0.5 hover:underline"
        >
      ...
      {groups.length === 0 ? (
        ...
        <div
          className="flex overflow-x-auto px-6 md:px-8 gap-4 pb-4 snap-x hide-scrollbar"
        >
          {groups.map((group) => {
            ...
            return (
              <div
                ...
                className="min-w-[150px] cursor-pointer bg-white rounded-[1.25rem] shadow-[0_4px_12px_rgba(0,0,0,0.03)] border border-gray-100 overflow-hidden flex flex-col snap-start hover:shadow-[0_10px_25px_rgba(0,0,0,0.06)] hover:-translate-y-1 transition-all duration-300"
              >
                <div
                ...
                <div className="p-4">
                  <h3 className="font-bold text-[#1a2b3c] text-[15px] truncate mb-0.5">
                    {group.name}
                  </h3>
                  <p className="text-[11px] font-medium text-[#8e98a3] mb-3">
        ...
        <div className="flex flex-col gap-3">
          {recentActivities.slice(0, 4).map((act) => (
            <div
              key={act.id}
              onClick={() => router.push("/activity")}
              className="flex items-center gap-3.5 bg-white p-3.5 rounded-[1.25rem] border border-gray-100 shadow-[0_2px_10px_rgba(0,0,0,0.02)] hover:shadow-[0_8px_20px_rgba(0,0,0,0.06)] hover:bg-[#fcfdfd] transition-all duration-300 cursor-pointer group"
            >
              <div className="w-12 h-12 rounded-full bg-[#f4f7f5] group-hover:bg-white border border-gray-100 group-hover:border-[#e0ebe7] flex items-center justify-center text-xl shadow-sm transition-colors duration-300">
                {act.groupEmoji || "📄"}
              </div>
              <div className="flex-1 min-w-0">
                <h4 className="text-[14px] font-bold text-[#1a2b3c] truncate group-hover:text-[#335c52] transition-colors">
                  {act.description}
                </h4>
                <p className="text-[11px] text-[#8e98a3] flex items-center gap-1.5 font-medium mt-0.5 truncate">
                  {act.groupName} <span className="w-1 h-1 rounded-full bg-[#cbd5e1]"></span> Paid by{" "}
                  {act.iPaid ? "You" : act.paidBy}
                </p>
              </div>
              <div className="text-right flex-shrink-0">
                ...
                >
                  {act.myShare > 0 ? "+ " : act.myShare < 0 ? "- " : ""}
                  {formatCurrency(Math.abs(act.myShare))}
                </p>
  ...
  const renderAIBanner = () => (
    <div className="mt-8 md:mt-10 px-6 md:px-8 mb-20 md:mb-8">
      <div
        ...
        className="bg-gradient-to-r from-[#eff6f3] to-[#e4f0ea] rounded-[1.25rem] p-4 flex items-center relative overflow-hidden border border-[#d2e4dd] shadow-sm cursor-pointer hover:shadow-md hover:-translate-y-0.5 transition-all duration-300"
      >
        ...
        <div className="ml-2 flex-1 relative z-10">
          ...
          <p className="text-[11px] text-[#5c6e7d] mt-0.5 mb-2.5 leading-tight font-medium pr-2">
            Let AI suggest the easiest way to settle up or scan bills.
          </p>
        </div>
    ...
    <div className="w-full bg-[#fdfaf5] min-h-screen relative flex flex-col md:flex-row overflow-hidden font-sans">
      ...
      <div className="hidden md:flex flex-col w-[260px] bg-white border-r border-gray-100 h-screen sticky top-0 py-8 px-6 z-20">
        ...
        <div
          ...
          className="flex items-center mb-10 px-0 cursor-pointer"
        >
          <img
            src="/logo.png"
            alt="SplitWise Logo"
        ...
        <div className="mt-auto pt-6 border-t border-gray-100">
          <button
            ...
            className="w-full bg-[#335c52] text-white py-3.5 rounded-[1rem] font-bold flex items-center justify-center gap-2 hover:bg-[#284a42] transition-all shadow-[0_8px_20px_rgba(51,92,82,0.25)]"
          >
      ...
      <div className="w-full max-w-lg mx-auto md:max-w-none md:flex-1 md:h-screen md:overflow-y-auto flex flex-col relative pb-28 md:pb-8 hide-scrollbar">
          ...
          <div className="flex items-center gap-5">
            ...
            >
              <div className="w-10 h-10 rounded-full bg-white border border-gray-100 flex items-center justify-center shadow-sm overflow-hidden group-hover:shadow-md transition-shadow">
                <img
                  src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${session?.user?.name || "user"}`}
                  alt="Profile"
                  className="w-full h-full object-cover"
                />
              </div>
              <div className="flex flex-col">
                <span className="text-[13px] font-bold text-[#1a2b3c] group-hover:text-[#335c52] transition-colors">
        ...
        <div className="w-full md:max-w-3xl md:mx-auto md:relative">
          ...
          <div className="flex md:hidden items-center justify-between px-6 pt-12 pb-4 relative z-20">
            <button
              onClick={() => router.push("/groups")}
              className="w-11 h-11 rounded-full bg-white shadow-[0_2px_10px_rgba(0,0,0,0.03)] border border-gray-100 flex items-center justify-center hover:bg-gray-50 transition-colors"
            >
            ...
            <div className="flex items-center gap-3">
              ...
              <div
                onClick={() => router.push("/profile")}
                className="w-11 h-11 rounded-full bg-[#1a2b3c] overflow-hidden flex items-center justify-center text-white font-bold shadow-sm border border-gray-200 cursor-pointer"
              >
          ...
          <div className="px-7 md:px-10 relative z-10 mt-6 md:mt-4 max-w-[70%]">
            <h1 className="text-[1.75rem] md:text-[2.5rem] font-bold text-[#1a2b3c] mb-1.5 md:mb-3 tracking-tight">
              {getGreeting()}, <span className="text-[#335c52]">{firstName}</span>! 👋
            </h1>
            <p className="text-[#8e98a3] text-[15px] md:text-[17px] font-medium">
              Let&apos;s settle up and stress less.
            </p>
          </div>
          ...
          <div className="absolute top-[0px] right-[-30px] md:top-[60px] md:right-[-120px] w-[360px] h-[360px] md:w-[420px] md:h-[420px] z-0 pointer-events-none group">
            <div
              className="absolute inset-0 bg-no-repeat transition-transform duration-1000 ease-out group-hover:scale-[1.03] group-hover:-translate-y-1"
              style={{
            ...
            <div className="relative md:max-w-[90%] group/card">
              <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] p-7 md:p-10 shadow-[0_10px_40px_rgba(0,0,0,0.05)] border border-gray-50 relative z-10 w-[95%] md:w-full group-hover/card:shadow-[0_20px_50px_rgba(0,0,0,0.08)] transition-shadow duration-500">
                ...
                <div className="flex items-baseline gap-2 md:gap-3 mb-1 md:mb-2">
                  <span className="text-[2.4rem] md:text-[3.5rem] font-bold text-[#335c52] tracking-tight">
                ...
                <p className="text-[13px] md:text-[16px] text-[#8e98a3] mb-7 md:mb-10 font-medium">
                  Across {groups.length} groups
                </p>

                <button
                  onClick={() => router.push(groups[0] ? `/groups/${groups[0].id}` : "/groups")}
                  className="bg-[#335c52] text-white px-7 md:px-9 py-3 md:py-4 rounded-full font-bold text-[13px] md:text-[16px] flex items-center gap-2 hover:bg-[#2a4d44] transition-colors shadow-sm md:shadow-md cursor-pointer"
                >
...
function ActionButton({
  ...
  return (
    <div className="flex flex-col items-center gap-2.5 md:gap-4 cursor-pointer group" onClick={onClick}>
      ...
      <span className="text-[12px] md:text-[14px] font-bold text-[#1a2b3c] text-center leading-tight px-1 group-hover:text-[#335c52] transition-colors">
        {label}
      </span>
    </div>
  );
  ...
  return (
    <div
      ...
      className={`flex items-center gap-3 px-4 py-3.5 rounded-xl cursor-pointer transition-colors ${
        active
          ? "bg-[#f0f6f4] text-[#335c52]"
          : "text-[#8e98a3] hover:bg-gray-50 hover:text-[#1a2b3c]"
      }`}
    >
